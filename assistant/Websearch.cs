    ...
    public partial class Websearch : Form
    {
        ...
        public Websearch()
        {
            ...
            try
            {
                ...
                george.SpeakCompleted += new EventHandler<SpeakCompletedEventArgs>(computer_SpeakCompleted);

                assistant.Properties.Settings.Default.Save();
                //georgeJPA.websearchpath = Properties.Settings.Default.WebS;
                //georgeJPA.websearchkeypath = Properties.Settings.Default.WebSK;
                //if (!File.Exists(georgeJPA.websearchpath))
                //{
                //    sw = File.CreateText(georgeJPA.websearchpath); sw.Write("Open Google"); sw.Close();
                //}
                //if (!File.Exists(georgeJPA.websearchkeypath))
                //{
                //    sw = File.CreateText(georgeJPA.websearchkeypath); sw.Write("Google"); sw.Close();
                //}
                //ArrayWebSearchCommands = File.ReadAllLines(georgeJPA.websearchpath); //This loads all written commands in our Custom Commands text documents into arrays so they can be loaded into our grammars
                //ArrayWebKeywordSearch = File.ReadAllLines(georgeJPA.websearchkeypath);
                loadGrammarAndCommands();
                if (george.State == SynthesizerState.Speaking)
                    george.SpeakAsyncCancelAll();
                foreach (InstalledVoice voice in george.GetInstalledVoices())
                {
        ...
        private void WebSearch_SpeechRecognized(object sender, SpeechRecognizedEventArgs e)
        {
            ...
            try
            {
                foreach (string line in ArrayWebSearchCommands)
                {
                    if (line == speech)
                    {
                        george.SpeakAsync(ArrayWebKeywordSearch[i]);
            ...
            {
                i += 1;
                george.SpeakAsync("Please check the " + speech + "web command on line " + i + ". It appears to be missing a proper response or web key words");
            }
        }
        ...
        private void loadGrammarAndCommands()
        {
            try
            {
                ...
                { george.SpeakAsync("I've detected an in valid entry in your web commands, possibly a blank line. web commands will cease to work until it is fixed."); }
            }
            catch (Exception ex)
            {
                throw ex;
            ...
            switch (speech)
            {
                ...
                case "change voice to brian":
                    ...
                    if (cbVoice.Text != "IVONA 2 Brian OEM")
                    {
                        ...
                        george.SelectVoice("IVONA 2 Brian OEM");
                    }
                    else
                    {
                        cbVoice.SelectedItem = "Microsoft David Desktop";
                        george.SelectVoice("Microsoft David Desktop");
                        george.Speak("ivona 2 brian is, not installed, here is microsoft david desktop, at your service");
                    }
                    break;
                case "change voice to salli":
                    if (cbVoice.Text != "IVONA 2 Salli OEM")
                    {
                        ...
                        george.SelectVoice("IVONA 2 Salli OEM");
                    }
                    else
                    {
                        cbVoice.SelectedItem = "Microsoft Zira Desktop";
                        george.SelectVoice("Microsoft Zira Desktop");
                        george.Speak("Ivona salli, is not installed, here is microsoft zira desktop, at your service");
                    }
                    break;
                ...
                case "change voice to amy":
                    if (cbVoice.Text != "IVONA 2 Amy OEM")
                    {
                        ...
                        george.SelectVoice("IVONA 2 Amy OEM");
                    ...
                    else
                    {
                        cbVoice.SelectedItem = "Microsoft Zira Desktop";
                        george.SelectVoice("Microsoft Zira Desktop");
                        george.Speak("Ivona amy, is not installed, here is microsoft zira desktop, at your service");
                    }
                    break;
        ...
        private void btnPause_Click(object sender, EventArgs e)
        {
            ...
            {
                if (george.State == SynthesizerState.Paused)
                {
                    george.Resume();
                    btnPause.Text = "Pause";
                }
